...
**Contact FAB + form (added 2026-07-06, jocaibe's floating companion):** ContactFab in the root layout — fixed bottom-right "Say hola" pill (with a mini RotatingAccent that spins with scroll). Appears after 500px of scroll, hides while #contact is in view. Opens a modal dialog (AnimatePresence, Escape/backdrop close, data-lenis-prevent, html overflow lock) whose submit composes a mailto: to j.cleon695@gmail.com from the name/email/message fields — no backend on the static site. Inputs keep cursor:text under the custom cursor. Upgrade path if real submissions are ever wanted: Formspree or a route handler + email service.
