...
**LC monogram (added 2026-07-06):** logo is the letters LC in Sora Bold on a navy rounded tile. Favicon via App Router convention: `src/app/icon.png` (512px, transparent corners; default favicon.ico removed) — regenerate with scratchpad logo.html + puppeteer if the brand changes. A ghost version (plain text LC, text-navy/[0.06], blur-md, 24rem, lg+ only) sits in the hero between the gradient blob and the DotGrid, so the cursor reveals dots on top of it.
